...
def cache_get_tts_audio_mono(text: str, lang='en') -> torch.Tensor:
    return get_tts_audio_mono(text, lang)
...
def get_tts_audio_source(text: str, lang='en', cache=False) -> PCMAudio:
    buf = io.BytesIO()
    mono = cache_get_tts_audio_mono(
        text, lang) if cache else get_tts_audio_mono(text, lang)
    mono = resampler(mono)
